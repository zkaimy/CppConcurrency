...
- **clangd 补全**：`compile_commands.json` 仅 Ninja/Makefile 生成器支持。VS 用户需额外执行一次 Ninja 配置：`cmake --preset windows-ninja`，生成在 `out/build/windows-ninja/compile_commands.json`。
